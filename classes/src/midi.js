...
(function (global) {
    ...
    var start_time = 0;
    var audio_status = "";
    var pause_total_time = 0;
    var pauseTime = 0;

    var alreadyLoaded = false, playing = false;
    ...
    function getTime() {
        return playing ? context.currentTime - start_time - pause_total_time : context.currentTime - pauseTime;
    }

    function get_next_wave(ev) {
        var player_event = new Object();
        player_event.time = context.currentTime - start_time - pause_total_time;
    ...
    function customLoadMissingPatch(path, filename) {
        var request = new XMLHttpRequest();
        request.open('GET', path + filename, true);
        request.responseType = 'arraybuffer';
        ...
        request.onload = function() {
            ...
            Module['FS_createDataFile']('pat/', filename, new Int8Array(request.response), true, true);
            MIDIjs.message_callback("Loading instruments: " + num_missing);
            if (num_missing == 0) alreadyLoaded = true;
        }
        request.send();
    }
    ...
    function load_WebAudioAPI_from_buffer(buf) {
        midiFileArray = new Int8Array(buf);
        ...
        console.log('MIDI length: ' + MIDIjs.length + ' s');
        ...
        if (0 < num_missing) {
            for (var i = 0; i < num_missing; i++) {
                ...
                customLoadMissingPatch(midijs_url + "pat/", missingPatch);
            }
        } else {
            alreadyLoaded = true;
        }
    ...
    function load_WebAudioAPI_from_url(url) {
        var request = new XMLHttpRequest();
        ...
        request.onload = function() {
            ...
            load_WebAudioAPI_from_buffer(request.response);
        }
        request.send();
    }

    function load_libtimidity(callback) {
        var libtimidity_url = "";
        midijs_url = 'https://acgsounds.com/javascripts/external/'; 
    ...
    function load(midi) {
        stop_WebAudioAPI();
        if (!Module) load_libtimidity(function() {
            if (midi instanceof ArrayBuffer)
                load_WebAudioAPI_from_buffer(midi);
            else load_WebAudioAPI_from_url(midi);
        });
        else 
            if (midi instanceof ArrayBuffer)
                load_WebAudioAPI_from_buffer(midi);
    ...
    function play_WebAudioAPI() {
        if (!alreadyLoaded) {
            MIDIjs.message_callback('No MIDI File loaded');
            return;
        }
        stream = Module.ccall('mid_istream_open_mem', 'number',  ['number', 'number', 'number'], [midiFileBuffer, midiFileArray.length , false]);
        var MID_AUDIO_S16LSB = 0x8010; // signed 16-bit samples
    ...
    function resume_WebAudioAPI(){
        playing = true;
        pause_total_time += (new Date - pauseTime) / 1000;
